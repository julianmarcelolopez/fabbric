...
- [ ] `DashboardPage.tsx:216` (barra "Catálogo"): `background: "#2563eb"`
      → `background: "#F07058"` (coral).
- [ ] `DashboardPage.tsx:225` (barra "Personalizado"): `background:
      "#7c3aed"` → `background: "#1E2A4A"` (navy) — se reusan los dos
      únicos colores de marca ya existentes, no se inventa un tercero.
- [ ] `DashboardPage.tsx:246` (3 barras de "Ventas por canal", hoy todas
      `#0891b2`): sacar el `background` inline sin reemplazo — hereda el
...
- [ ] En "Catálogo vs Personalizado" las dos barras se distinguen a simple
      vista (coral vs. navy).
- [ ] En "Ventas por canal" las 3 barras se ven coral — mismo nivel de
