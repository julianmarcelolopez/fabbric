...
2. **Escanear** (destino de la bottom nav) — pantalla principal. Un selector de modo, **"Vender" / "Recibir mercadería"** (agregado en T27, capa de interacción — no cambia ningún endpoint), determina qué pasa si el código no existe: en modo "Recibir mercadería" pasa a "Alta de producto", igual que antes de T27; en modo "Vender" se avisa con un mensaje en pantalla y no se navega a Alta, para no dar de alta un producto nuevo por error en medio de una venta. Abre la cámara del celular (vía navegador) y lee el código de barras. Busca contra `GET /admin/variants/by-barcode/:code`. Si el código existe, pasa a "Ficha de producto" en cualquiera de los dos modos.
...
4. **Ficha de producto** (estado, no destino — se llega solo si el código escaneado ya existe) — muestra foto, marca, talle, color, precio y `stockLocal` actual. Según el modo elegido en Escanear (T27), se ve **una sola** de estas dos acciones, no las dos juntas — ambas vuelven a Escanear al ejecutarse, con una confirmación de pantalla completa (agregada en T27):
   - Modo "Recibir mercadería" → **Registrar entrada**: `POST /admin/variants/:id/stock-movements` (`channel: local`, `type: entrada`).
   - Modo "Vender" → **Agregar a la venta**: suma el producto al carrito en curso (estado local de la app, no se persiste hasta confirmar) y actualiza el contador en el ícono de Carrito de la bottom nav.
